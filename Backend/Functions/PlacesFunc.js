...
const insert_data = async (req, res) => {
  try {
    const data_store = await Places.insertMany(hotel_data);
...
const give_based = async (req, res) => {
  const { country } = req.body;
  try {
    const data_country = await Places.find({ country: country });
    return res.status(201).json({ message: data_country });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
};
const popular_review = async (req, res) => {
  const { values, country } = req.body;
  try {
    const ans = await Places.find({
      country: country,
      star_rating: { $gte: values },
    });

    return res.status(201).json({ message: ans });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
};
const popular_review_continent = async (req, res) => {
  const { continent_name } = req.body;
  try {
    const ans = await Places.find({
      continent_name: continent_name,
    });
    ans.sort((a, b) => b.star_rating - a.star_rating);
    return res.status(201).json({ message: ans });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
};
const get_continent = async (req, res) => {
  try {
    const { continent_name } = req.body;
    if (continent_name) {
      const ans = await Places.find({ continent_name: continent_name });
...
const get_values = async (req, res) => {
  try {
    const ans = await Places.find({}, { continent_name: 1, _id: 0 });
    return res.status(201).json({ message: ans });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
...
const get_cities = async (req, res) => {
  try {
    const ans = await Places.aggregate([
      { $group: { _id: "$city" } }, // Group by city to get unique cities
    ]);
    ans.sort((a, b) => a._id.localeCompare(b._id));
    return res.status(201).json({ message: ans });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
};
const get_unique_countries = async (req, res) => {
  try {
    const countries = await Places.aggregate([{ $group: { _id: "$country" } }]);
    countries.sort((a, b) => a._id.localeCompare(b._id));
    return res.status(201).json({ message: countries });
  } catch (err) {
    return res.status(401).json({ message: err });
  }
};
const getby_id = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: place });
  } catch (err) {
    return res.status(401).json({ message: "Internal server error" });
  }
};
const get_cities_name = async (req, res) => {
  try {
    const { city_name } = req.body;
    const place = await Places.find({ city: city_name });
    return res.status(201).json({ message: place });
  } catch (err) {
